...
```python
V_SLIP_MIN = 5.0            # Minimum slip velocity for thrust (m/s)
V_SLIP_MAX = 100.0          # Maximum slip velocity to limit losses (m/s)
SLIP_RATIO_NORMAL = 0.02    # Target slip ratio at full current (2%)
SLIP_RATIO_REDUCED = 0.01   # Target slip ratio when power-limited (1%)
```

**Three operating regimes:**

| Regime | v_rel Range | Behavior |
|--------|-------------|----------|
| STARTUP | < ~250 m/s | v_slip clamped at V_SLIP_MIN, slip ratio naturally high |
| SWEET SPOT | ~250-5000 m/s | v_slip follows slip ratio target, thrust power dominates |
| HIGH SPEED | > ~5000 m/s | Power controller limits current, slip tapers to reduce losses |

When the power controller reduces current (due to cryo or power limits), the slip target tapers from 2% down to 1%. This reduces eddy losses, allowing more current headroom for thrust.
