...
# γ-TiAl (gamma titanium aluminide) - current default plate material
RHO_gTI_293K = 75e-8              # Resistivity at 293 K (Ω·m)
ALPHA_gTI = 0.0012                # Temperature coefficient (1/K)

# Calculate resistivity at typical operating temperature (~350 K in cable mode)
T_OPERATING = 350  # K
RHO_PLATE = RHO_gTI_293K * (1 + ALPHA_gTI * (T_OPERATING - 293))
SIGMA_PLATE = 1.0 / RHO_PLATE
...
class LIMParams:
    ...
    - 3mm × 1-layer HTS tape → I_c = 200 A
    - N = 200 turns
    - τp = 100 m, W = 2.0 m, gap = 0.20 m
    - γ-TiAl plate, 200 mm thick
    - 8 MW site power limit
    """
    
    # === PRIMARY DESIGN VARIABLES (what we're optimizing) ===
    N: int = 200                  # Turns per coil
    W: float = 2.0                # Coil width (m)
    tau_p: float = 100.0          # Pole pitch (m)
    d_kapton: float = 1e-3        # Kapton thickness (m)
    
    # === SECONDARY PARAMETERS (fixed or constrained) ===
    pitch_count: int = 3          # Pole pitches per LIM
    gap: float = 0.20             # Air gap (m)
    t_plate: float = 0.20         # Reaction plate thickness (m)
    d_HTS: float = 80e-6          # HTS tape thickness (m)
    
    # === OPERATING PARAMETERS ===
    I_target: float = 163.0       # Target current (A) - 81.25% of I_c for 3mm×1 layer
    v_slip: float = 200.0         # Slip velocity (m/s)
    
    # === LIMITS ===
    I_c: float = 200.0            # HTS critical current (A) - 3mm × 1 layer
...
def calc_skin_depth(f_slip: float) -> float:
    """Skin depth in γ-TiAl reaction plate (m)"""
    if f_slip <= 0:
        return 0.1
    return math.sqrt(RHO_PLATE / (math.pi * MU0 * f_slip))
...
def calc_goodness_factor(p: LIMParams, f_slip: float) -> float:
    ...
    return (omega_slip * MU0 * SIGMA_PLATE * delta_eff * p.tau_p) / math.pi
...
def main():
    ...
    # Baseline design - matches lim_config.py (January 2026)
    baseline = LIMParams(
        N=200,
        W=2.0,
        tau_p=100.0,
        ...
        gap=0.20,
        t_plate=0.20,
        I_target=163,       # 81.25% of I_c for 3mm × 1 layer
        I_c=200,            # 3mm × 1 layer HTS tape
        v_slip=200,
        P_limit_site=8e6,
    )
    
    # We'll analyze at two key points: early and late deployment
    v_early = 500    # Thrust-limited regime (early deployment)
    v_late = 8000    # Power-limited regime (late deployment, approaching orbital velocity)
    
    print(f"""
CURRENT BASELINE (from lim_config.py):
- HTS tape: 3mm × 1 layer → I_c = 200 A, I_target = 163 A
- N = 200 turns, τp = 100 m, W = 2.0 m
- Gap = 200 mm, Plate = 200 mm γ-TiAl
- Site power limit = 8 MW
...
""")
    ...
    print("=" * 70)
    print(f"""
BASELINE PARAMETERS (lim_config.py January 2026):
  N = {baseline.N} turns
  W = {baseline.W} m
  τp = {baseline.tau_p} m
  gap = {baseline.gap} m
  t_plate = {baseline.t_plate} m (γ-TiAl)
  I_c = {baseline.I_c} A (3mm × 1 layer)
...
2. COIL WIDTH (W):
   ...
   - Sweet spot around W = 2 m for these parameters
...
3. POLE PITCH (τp):
   ...
   - τp = 100 m is a good compromise for orbital ring scale
...
RECOMMENDATIONS:
   - Keep N = 200 (current baseline, good voltage margin)
   - Keep W = 2 m (good balance of field and area)
   - Keep τp = 100 m (practical for orbital ring scale)
   - Keep gap = 200 mm (mechanical clearance for levitation)
